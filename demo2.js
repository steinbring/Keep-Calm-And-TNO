// Next to the "Encrypted Message" text area, there is a spot for a link with the "data URL" as the href. This refreshes that value
var refreshDataURLLink = function(message,LinkID) {
	document.getElementById(LinkID).innerHTML = '<a href="'+message+'">The File</a>';
};
// This grabs the file contents when the file changes
var openFile = function(event,TextAreaID,LinkID) {
	...
	reader.onload = function(){
		...
		// Update the output to include a link with the data URL as the href
		refreshDataURLLink(TheFileContents,LinkID);
		// Update the text area to include the contents of the file
		document.getElementById(TextAreaID).value = TheFileContents;
	};
...
var decryptMessage = function(encryptedContent,key,passPhrase){
	...
	openpgp.decryptMessage(privateKey, pgpMessage).then(function(plaintext) {
		// Update the 'Plaintext Message' textarea
	    setPTMessage(plaintext);
	    // Update the output to include a link with the data URL as the href
		refreshDataURLLink(plaintext,'ThePTLink');
	}).catch(function(error) {
		// Failure
